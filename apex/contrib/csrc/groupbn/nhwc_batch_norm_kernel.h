...
DEVICE_FUNCTION void from_float(int (&dst)[N], const float (&src)[2*N]) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        asm volatile("v_cvt_f16_f32( %h0, %1)" : "=v"(lo) : "v"(src[2*i+0]));
        asm volatile("v_cvt_f16_f32( %h0, %1)" : "=v"(hi) : "v"(src[2*i+1]));
        asm volatile("v_mov_b32(%h0, %1, %2)" : "=v"(dst[i]) : "v"(lo), "v"(hi));
    }
}
...
DEVICE_FUNCTION void to_float(float (&dst)[2*N], int (&src)[N]) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        asm volatile("v_mov_b32((%h0, %1), %2)" : "=v"(lo) : "v"(hi), "v"(src[i]));
        asm volatile("v_cvt_f16_f32( %h0, %1)" : "=v"(dst[2*i+0]) : "v"(lo));
        asm volatile("v_cvt_f16_f32( %h0, %1)"   : "=v"(dst[2*i+1])   : "v"(hi));
    }
}
...
DEVICE_FUNCTION void ldg_stream(int (&dst)[1], const uint16_t *gmem) {
    ...
    asm volatile ("flat_load_dword %0, [%1];"  : "=v"(tmp) : "v" ((const uint *)gmem));
    dst[0] = tmp;
}
...
DEVICE_FUNCTION void ldg_stream(int (&dst)[2], const uint16_t *gmem) {
    ...
    asm volatile ("flat_load_dwordx2 {%0,%1}, [%2];"
        : "=v"(tmp.x), "=v"(tmp.y) : "v"((const int2 *)gmem));
    dst[0] = tmp.x;
    dst[1] = tmp.y;
}
...
DEVICE_FUNCTION void stg_stream(uint16_t *gmem, int (&src)[1]) {
    ...
    //    :: "l"((uint *)gmem) , "r"(tmp));
    asm volatile ("flat_store_dword [%0], %1;" :: "v"((uint *)gmem) , "v"(tmp));
}
...
DEVICE_FUNCTION void stg_stream(uint16_t *gmem, int (&src)[2]) {
    ...
    asm volatile ("flat_store_dwordx2 [%0], {%1,%2};"
        :: "v"((uint *)gmem) , "v"(src[0]), "v"( src[1]));
}
...
DEVICE_FUNCTION void parallel_sums_16x2(float *smem, float (&x)[4], int nhw,
    ...
    for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
        ...
        x[i] += __shfl(x[i], THREADS_PER_PIXEL+lane_id);
    }
...
DEVICE_FUNCTION void parallel_sums_8x4(float *smem, float (&x)[4], int nhw) {
    ...
    for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
        //x[i] += __shfl_sync(0xffffffffU, x[i], THREADS_PER_PIXEL+lane_id);
        //x[i] += __shfl_sync(0xffffffffU, x[i], THREADS_PER_PIXEL+lane_id);
        x[i] += __shfl(x[i], THREADS_PER_PIXEL*2+lane_id);
        x[i] += __shfl(x[i], THREADS_PER_PIXEL*2+lane_id);
    }
    ...
    if (threadIdx.x == 0) {
        ...
        do {
            ...
            asm volatile ("flat_load_dword %0, [%1];"
                : "=v"(retired_ctas) : "v"(gmem_retired_ctas));
            ...
            for (int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i) {
                ...
                if (USE_ADD_RELU) {
                    ...
                    for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
                        ...
                        unsigned int local_relu_mask = __ballot(rectified);
                        if (lane_id == i) {
    ...
    for (int c_blk_index = blockIdx.y; c_blk_index < params.c_blks; c_blk_index += gridDim.y) {
        ...
        if (pixels_in_smem > 0) {
            ...
            for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
                ...
                #pragma unroll
                for (int j = 0; j < ELEMENTS_PER_LDG; ++j) {
                    rectified[j] = ((__shfl(relu_mask, j) &
                                    (1U << lane_id)) != 0);
                }
