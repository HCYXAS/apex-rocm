...
class NhwcBatchNorm {
 public:
  ...
  void setInputDescriptor(const hipdnnTensorFormat_t format,
  ...
  void setOutputDescriptor(const hipdnnTensorFormat_t format,
                                   const hipdnnDataType_t     data_type,
  ...
  void processCudnnStatus(const hipdnnStatus_t& status,
                          const std::string& string = std::string(),
                          bool verbose = VERBOSE_DEFAULT) {
    if (status != HIPDNN_STATUS_SUCCESS)
      LOG(FATAL) << string << " " << hipdnnGetErrorString(status);
    else if (verbose)
      LOG(INFO) << string << " " << hipdnnGetErrorString(status);
  }
  ...
  hipdnnTensorDescriptor_t  X_tensor_desc_ = nullptr;
  hipdnnTensorDescriptor_t  Y_tensor_desc_ = nullptr;
 ...
 private:
  void setTensorDescriptor(hipdnnTensorDescriptor_t descriptor,
                           hipdnnTensorFormat_t format,
                           hipdnnDataType_t     data_type,
                           int n, int c, int h, int w) {
    hipdnnStatus_t status = HIPDNN_STATUS_SUCCESS;
    status = hipdnnSetTensor4dDescriptor(descriptor, format, data_type, n, c, h, w);
    processCudnnStatus(status, "set tensor descriptor");
  }

  void createTensorDescriptor(hipdnnTensorDescriptor_t *descriptor) {
    hipdnnStatus_t status = HIPDNN_STATUS_SUCCESS;
    status = hipdnnCreateTensorDescriptor(descriptor);
    processCudnnStatus(status, "create tensor_descriptor");
  }

  void destroyTensorDescriptor(hipdnnTensorDescriptor_t descriptor) {
    hipdnnStatus_t status = HIPDNN_STATUS_SUCCESS;
    status = hipdnnDestroyTensorDescriptor(descriptor);
...
template <typename... Args>
  void _fwdKernelLauncher(hipStream_t stream, dim3 grid_dim, int outer_loops, bool use_relu, const int occupancy, const bool coop, Args... args) {
...
template <typename... Args>                              
  void _bwdKernelLauncher(hipStream_t stream, dim3 grid_dim, int outer_loops, bool use_relu, const int occupancy, const bool coop, Args... args) {
...
void NhwcBatchNorm::fwdInference(hipStream_t stream, bool use_relu) {
  ...
  if (use_relu) {
    hipLaunchKernelGGL(nhwc_batch_norm_fwd_inference<StorageType, THREADS_PER_CTA, THREADS_PER_PIXEL, ELEMENTS_PER_LDG, true, false>, dim3(grid_dim), dim3(THREADS_PER_CTA), 0, stream, params);
    checkCudaStatus(name_ + " fwd_inference-relu kernel");
  } else {
    hipLaunchKernelGGL(nhwc_batch_norm_fwd_inference<StorageType, THREADS_PER_CTA, THREADS_PER_PIXEL, ELEMENTS_PER_LDG, false, false>, dim3(grid_dim), dim3(THREADS_PER_CTA), 0, stream, params);
...
void NhwcBatchNorm::fwd(hipStream_t stream, bool use_relu, void* my_data, void* pair_data, void* pair_data2, void* pair_data3,
  ...
  _fwdKernelLauncher(stream, grid_dim, params.outer_loops, use_relu, occupancy, coop, 
                     params.gmem_src,         
		     params.gmem_dst,         
		     params.gmem_src1,        
		     params.gmem_bias,        
		     ...
		     params.gmem_running_var, 
		     params.gmem_saved_mean,  
		     params.gmem_saved_var,   
	  	     params.gmem_relu_bitmask,
		     params.nhw,              
		     params.c,                
		     params.svar_inv_count,   
		     params.rvar_inv_count,   
		     params.gmem_sums,        
		     params.gmem_counts,      
		     params.gmem_retired_ctas,
		     params.var_eps,          
		     params.outer_loops,      
		     params.exp_avg_factor,   
		     params.c_blks,
                     params.my_data,
                     params.pair_datas[0],
                     params.pair_datas[1],
...
void NhwcBatchNorm::dgrad(hipStream_t stream, bool use_relu, void* my_data, void* pair_data, void* pair_data2, void* pair_data3, 
  ...
  _bwdKernelLauncher(stream, grid_dim, params.outer_loops, use_relu, occupancy, coop,
                     params.gmem_src,
                     params.gmem_dy,          
                     params.gmem_dst,         
                     params.gmem_dst1,        
